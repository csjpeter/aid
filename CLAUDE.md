...
- `~/.claude`, `~/.local/share/claude-userdata`, `~/.copilot`, `~/.gemini`, `~/.tmuxinator` and `~/.config/nvim` shared host→guest via virtiofs (`kvm-share.sh`); tags: `claude`, `claude-userdata`, `copilot`, `gemini`, `tmuxinator`, `nvim-config`
- `~/.pi` shared via virtiofs only when Pi coding agent is installed (tag: `pi`)
